...
Opciones:
  -i, --input=<ruta>     CSS de entrada (entry de Panda). Default: pum/index.css
  -o, --output=<ruta>    Ruta de salida. Default: www/styles.css
...
Ejemplos:
  ...
  bun run scripts/build-css.ts -o=www/styles.css --target=all
...
function parseArgv(argv: string[]): CliOptions {
  const opts: CliOptions = {
    input: resolve(ROOT, 'pum/index.css'),
    output: resolve(ROOT, 'www/styles.css'),
    targets: SAFARI_2024,
    minify: true,
    help: false,
  }
...
async function main() {
  ...
  // pum/index.css es el entry de Panda: su primera línea
